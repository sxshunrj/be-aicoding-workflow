...
首次使用：左侧「＋ 添加仓库」注册含 `.ai-workflow.yaml` 的仓库 → 发起 Run → 详情页点「▶ 驱动 agent」由 GUI 无人值守指挥 agent 执行（命令模板在「装机与诊断 → Agent 命令」配置），或在终端手动跑。到达审批 gate 时回 GUI 审批——接受后自动接力继续驱动。GUI 重启不影响运行中的 agent（重启后自动重新接管）。前端源码在 `gui/`（React + Vite），构建产物已包含在包内（`src/ai_workflow_gui/static/`），使用方无需 Node；改前端后 `cd gui && npm run build` 重新构建并提交产物。
