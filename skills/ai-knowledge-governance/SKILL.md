...
展示人类选择前，若已配置通知通道，调用 `ai-workflow wecom notify --repo <repo> [--run-id <run-id>] --gate governance --subject <candidate-id> --action "请选择 promote / reject / 保持"` 通知团队；失败仅写 warning，不影响主流程。`--subject` 必须传候选知识 id（如 `KW-pattern-xxx-001`）：candidate 产生时 Helper 已通过 `reflect-submit` / `wiki propose` 按同一 subject 机械推送并跨 run 去重，已推送过则此处自动去重不再打扰群；此前从未推送成功过（如当时通知软失败）才会真正发出，保证候选一定被团队看到。`<repo>` 为当前仓库路径；`--run-id` 仅在当前处于某个 run 内时传（run 外治理可省略）。
